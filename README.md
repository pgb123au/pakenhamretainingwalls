# Pakenham Retaining Walls — rank-and-rent microsite
...
| Item | Value |
|---|---|
| Domain | `pakenhamretainingwalls.com.au` (registered, GoDaddy fraud-pending) |
| Live URL (interim) | https://pakenhamretainingwalls.netlify.app |
| Phone | **(03) 9003 0223** (Telnyx, Melbourne) |
...
│   ├── officer.astro         # /officer/
│   ├── berwick.astro         # /berwick/
│   ├── beaconsfield.astro    # /beaconsfield/
│   ├── cardinia.astro        # /cardinia/ (shire-wide overview)
...
├── whisper.js                # "Pete's Lead from Pakenham Retaining Walls" played to contractor before patching caller through
...
| Telnyx number | `+61390030223` (id `2936965401516443297`) |
...
| `TENANT_BRAND` | `Pakenham Retaining Walls` (default) | What plays after the label — full whisper is `"Pete's Lead. Pete's Lead from Pakenham Retaining Walls."` |
...
3. Submit to Google Search Console (`sc-domain:pakenhamretainingwalls.com.au`).
4. Set up Google Business Profile (needs registered business name "Pakenham Retaining Walls" first).
...
|---|---|
| Local fit | Pakenham/Officer estates are sloped — demand is structural, not weather-cyclical |
